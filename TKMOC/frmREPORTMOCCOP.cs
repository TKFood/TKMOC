...
namespace TKMOC
{
    public partial class frmREPORTMOCCOP : Form
    {
        ...
        public StringBuilder SETSQL1()
        {
            ...
            SB.AppendFormat(" ,CASE WHEN ISNULL(TA014,'')<>'' THEN DATEDIFF (DAY,[COPTD].TD013,TA014) ELSE 999 END AS '是否延遲訂單預交'");
            SB.AppendFormat(" ,CASE WHEN ISNULL(TA014,'')<>'' THEN DATEDIFF (DAY,TA010,TA014) ELSE 999 END  AS '是否延遲製令完工'");
